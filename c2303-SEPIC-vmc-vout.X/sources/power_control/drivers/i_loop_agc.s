...
;------------------------------------------------------------------------------
	...
	.global _i_loop_AGCFactorUpdate
_i_loop_AGCFactorUpdate:
    ...
    ; read output voltage 
    mov [w0 + #usrParam3], w2		    ; load pointer to most recent output voltage register
    mov [w2], w2                            ; load value into w2
    
    ; Calculate instantaneous VL (VIN + VOUT)
    add w1, w2, w5                          ; calculate most recent VL, place result in w5
